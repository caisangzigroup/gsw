...
class Upload extends CI_Controller 
{
	...
	{
		$now = time();
		$upload_path = './uploads/member/'.date("Y",$now).'/'.date("m",$now).'/';
		...
		echo json_encode(array(
			'name'  => $newname,
			'pic_url'	=> '/uploads/member/'.date("Y",$now).'/'.date("m",$now).'/'.$newname,
			'error' => $error,
		));
		die();
	}
	...
	public function upload_headpic()
    {
        ...
        else
        {
            ...
            $data = array('url'=>'/uploads/member/'.date("Y",$now).'/'.date("m",$now).'/'.$upload_data['file_name']);
